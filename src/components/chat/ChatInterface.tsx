...
export function ChatInterface({ videoId, onCitationClick }: ChatInterfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {messages.length === 0 ? (
        ...
        <div className="flex-1 space-y-6 mb-6 overflow-y-auto p-1">
          {messages.map((msg) => (
            <div key={msg.id} className={cn("flex", msg.role === "user" ? "justify-end" : "justify-start")}>
              <div className={cn("max-w-[80%]", msg.role === "user" ? "order-2" : "order-1")}>
                {msg.role === "assistant" && (
                ...
                {msg.citations && msg.citations.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {msg.citations.map((c, i) => (
                      <button
                        key={i}
                        onClick={() => onCitationClick?.(c.timestamp)}
                        className="flex items-start gap-2 text-xs text-white/40 bg-white/3 rounded-lg p-2 hover:bg-white/5 transition-colors w-full text-left"
                      >
